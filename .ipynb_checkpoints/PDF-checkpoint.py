...
st.title("Recherche et analyse de texte")
space(2) 
...
st.sidebar.markdown("> Le critère de recherche n'est pas sensible à la casse.")
st.sidebar.markdown("> Les accents ne doivent pas être oubliés (exemple : les mots 'école' et 'ecole' ne sont pas considérés comme équivalents et ne donneront pas les mêmes sorties).")
st.sidebar.markdown("> Certains mots (les stopwords) sont par défaut supprimés des fichiers")
st.sidebar.markdown("> La langue considérée est le français par défaut.")

#test if language has to be changed
space(2,side=True)
language = st.sidebar.selectbox("Choissisez la langue du texte", ["french", "english", "spanish"])
...
pdf_name = st.file_uploader("Choisissez un fichier")
...
stopwords_toadd = st.multiselect("Mots à supprimer", most_common_words.keys())
...
#word cloud
space(3)
number_of_word = st.slider("Select the number of words you want to plot", 5, 100)
